...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.knowledgebaseArticle.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.knowledgebase-articles.update", [$knowledgebaseArticle->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.knowledgebaseArticle.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $knowledgebaseArticle->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple required>
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $knowledgebaseArticle->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $knowledgebaseArticle->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('action') ? 'is-invalid' : '' }}" name="action" id="action">{!! old('action', $knowledgebaseArticle->action) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('gotcha') ? 'is-invalid' : '' }}" name="gotcha" id="gotcha">{!! old('gotcha', $knowledgebaseArticle->gotcha) !!}</textarea>

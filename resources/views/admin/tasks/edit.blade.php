...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="features">{{ trans('cruds.task.fields.feature') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('features') ? 'is-invalid' : '' }}" name="features[]" id="features" multiple>
                    @foreach($features as $id => $feature)
                        <option value="{{ $id }}" {{ (in_array($id, old('features', [])) || $task->features->contains($id)) ? 'selected' : '' }}>{{ $feature }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.feature_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="related_knowledges">{{ trans('cruds.task.fields.related_knowledge') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('related_knowledges') ? 'is-invalid' : '' }}" name="related_knowledges[]" id="related_knowledges" multiple>
                    @foreach($related_knowledges as $id => $related_knowledge)
                        <option value="{{ $id }}" {{ (in_array($id, old('related_knowledges', [])) || $task->related_knowledges->contains($id)) ? 'selected' : '' }}>{{ $related_knowledge }}</option>
                    @endforeach
                </select>
                @if($errors->has('related_knowledges'))
                    <div class="invalid-feedback">
                        {{ $errors->first('related_knowledges') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.related_knowledge_helper') }}</span>
            </div>
            <div class="form-group">

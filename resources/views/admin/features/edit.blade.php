    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.features.update", [$feature->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project') ? 'is-invalid' : '' }}" name="project_id" id="project_id" required>
                    @foreach($projects as $id => $entry)
                        <option value="{{ $id }}" {{ (old('project_id') ? old('project_id') : $feature->project->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('use_cases') ? 'is-invalid' : '' }}" name="use_cases[]" id="use_cases" multiple required>
                    @foreach($use_cases as $id => $use_case)
                        <option value="{{ $id }}" {{ (in_array($id, old('use_cases', [])) || $feature->use_cases->contains($id)) ? 'selected' : '' }}>{{ $use_case }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{!! old('notes', $feature->notes) !!}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Feature::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $feature->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('phases') ? 'is-invalid' : '' }}" name="phases[]" id="phases" multiple>
                    @foreach($phases as $id => $phase)
                        <option value="{{ $id }}" {{ (in_array($id, old('phases', [])) || $feature->phases->contains($id)) ? 'selected' : '' }}>{{ $phase }}</option>
                    @endforeach
                </select>

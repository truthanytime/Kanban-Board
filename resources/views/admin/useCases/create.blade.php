...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.use-cases.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('as_a') ? 'is-invalid' : '' }}" name="as_a_id" id="as_a_id" required>
                    @foreach($as_as as $id => $entry)
                        <option value="{{ $id }}" {{ old('as_a_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('as_a'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('i_want_to') ? 'is-invalid' : '' }}" name="i_want_to" id="i_want_to">{{ old('i_want_to') }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('so_i_can') ? 'is-invalid' : '' }}" name="so_i_can" id="so_i_can" required>{{ old('so_i_can') }}</textarea>

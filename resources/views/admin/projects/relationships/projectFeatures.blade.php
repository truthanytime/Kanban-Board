@can('feature_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.features.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.feature.title_singular') }}
            </a>
        </div>
    </div>
@endcan

<div class="card">
    <div class="card-header">
        {{ trans('cruds.feature.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-projectFeatures">
                <thead>
                    <tr>
                        <th width="10">

                        </th>
                        <th>
                            {{ trans('cruds.feature.fields.id') }}
                        </th>
                        <th>
                            {{ trans('cruds.feature.fields.project') }}
                        </th>
                        <th>
                            {{ trans('cruds.feature.fields.use_case') }}
                        </th>
                        <th>
                            {{ trans('cruds.feature.fields.feature_name') }}
                        </th>
                        <th>
                            {{ trans('cruds.feature.fields.status') }}
                        </th>
                        <th>
                            {{ trans('cruds.feature.fields.phase') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($features as $key => $feature)
                        <tr data-entry-id="{{ $feature->id }}">
                            <td>

                            </td>
                            <td>
                                {{ $feature->id ?? '' }}
                            </td>
                            <td>
                                {{ $feature->project->name ?? '' }}
                            </td>
                            <td>
                                @foreach($feature->use_cases as $key => $item)
                                    <span class="badge badge-info">{{ $item->i_want_to }}</span>
                                @endforeach
                            </td>
                            <td>
                                {{ $feature->feature_name ?? '' }}
                            </td>
                            <td>
                                {{ App\Models\Feature::STATUS_SELECT[$feature->status] ?? '' }}
                            </td>
                            <td>
                                @foreach($feature->phases as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @can('feature_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.features.show', $feature->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('feature_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.features.edit', $feature->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('feature_delete')
                                    <form action="{{ route('admin.features.destroy', $feature->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                    </form>
                                @endcan

                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>

@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('feature_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.features.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  let table = $('.datatable-projectFeatures:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection